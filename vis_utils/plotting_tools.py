...
# This implementation is mine
def plot_reliability_diagram(probabilities, target_labels, n_bins=10, task='binary', temp_attention:float=1., temp_classification:float=1.):
    """
    Plots a reliability diagram for binary classification. Currently only binary classification!!!! Don't try this for multiclass
    Args:
    probabilities (torch.Tensor): Probabilistic outputs from the model, shape (n_samples, n_classes)
    target_labels (torch.Tensor): True class labels, shape (n_samples,)
    n_bins (int): Number of bins to divide the probability interval into

    """
    assert task == 'binary', 'Currently only binary classification is supported'
    probabilities = probabilities.numpy()
    target_labels = target_labels.numpy()

    # Confidence here is the probability of the positive class
    confidences = probabilities.max(-1)
    pred_labels = np.argmax(probabilities, axis=-1)

    correctness = pred_labels == target_labels
    accuracy_per_bin = np.zeros(n_bins)
    confidence_per_bin = np.zeros(n_bins)

    bins = np.linspace(0, 1, n_bins + 1, endpoint=True)
    for bin_index in range(n_bins):
        in_bin = (confidences > bins[bin_index]) & (confidences <= bins[bin_index + 1])
        if np.sum(in_bin) > 0:
            accuracy_per_bin[bin_index] = np.mean(correctness[in_bin])
            confidence_per_bin[bin_index] = np.mean(confidences[in_bin])

    # Create and return the matplotlib Figure
    fig, ax = plt.subplots(figsize=(6, 6))
    ax.bar(bins[:-1], accuracy_per_bin, width=np.diff(bins), edgecolor='k', align='edge', label='Observed accuracy')
    ax.plot([0, 1], [0, 1], 'k--', label='Perfect calibration')

    # Add temperatures to legend
    legend_title = ''
    if temp_attention is not None:
        legend_title += f'Attention Temp: {temp_attention:.2f}\n'
    if temp_classification is not None:
        legend_title += f'Classification Temp: {temp_classification:.2f}'

    ax.legend(title=legend_title.strip())
    ax.set_xlim(0, 1)
    ax.set_ylim(0, 1)
...
def confidence_histogram(probabilities:torch.Tensor=None, target_labels:torch.Tensor=None, n_bins:int=20, temp:float=1.):
    """
    Plot a histogram of the confidence of the model's predictions.
    Args:
    probabilities (torch.Tensor): Probabilistic outputs from the model, shape (n_samples, n_classes)
    target_labels (torch.Tensor): True class labels, shape (n_samples,)
    n_bins (int): Number of bins to divide the probability interval into
    ...
    assert target_labels is not None, 'Target labels must be provided'

    # Confidence here is the probability of the positive class
    confidences = probabilities.max(dim=-1).values.numpy()
    pred_labels = np.argmax(probabilities, axis=-1)
    accuracy = accuracy_score(y_true=target_labels, y_pred=pred_labels)
    balanced_accuracy = balanced_accuracy_score(y_true=target_labels, y_pred=pred_labels)
    # print(f'Accuracy from conf histogram function: {accuracy}, Balanced Accuracy: {balanced_accuracy}')
    mean_confidence = np.mean(confidences)

    # Create a figure and axis
    # Change the font and fonsize of labels and text, fontfamily should be something researchy
    plt.rcParams.update({'font.size': 12, 'font.family': 'serif'})

    fig, ax = plt.subplots(figsize=(6, 6))
    bins = np.linspace(0, 1, n_bins + 1, endpoint=True)
    sns.histplot(data=confidences, bins=bins, ax=ax, stat='percent')
    offset = 0.02
    ax.axvline(balanced_accuracy, color='gray', linestyle='--', label=f'Balanced Accuracy: {balanced_accuracy:.2f}')
    ax.text(balanced_accuracy + offset, 98, 'Balanced Accuracy', rotation=90, va='top', ha='left', color='black')
    ax.axvline(mean_confidence, color='gray', linestyle='--', label=f'Mean Confidence: {mean_confidence:.2f}')
    ax.text(mean_confidence - offset, 98, 'Average Confidence', rotation=90, va='top', ha='right', color='black')
    ...
    temp_label = Patch(color='none', label=f'Softmax Temp: {temp:.2f}')
    handles.append(temp_label)
