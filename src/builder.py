...
def has_HOPTIMUS():
    HAS_HOPTIMUS = False
    HOPTIMUS_CKPT_PATH = ''
    # check if HOPTIMUS_CKPT_PATH is set, catch exception if not
    try:
        if 'HOPTIMUS_CKPT_PATH' not in os.environ:
            raise ValueError('HOPTIMUS_CKPT_PATH not set')
        HAS_HOPTIMUS = True
        HOPTIMUS_CKPT_PATH = os.environ['HOPTIMUS_CKPT_PATH']
    except Exception as e:
        print(e)
    return HAS_HOPTIMUS, HOPTIMUS_CKPT_PATH
...
def get_encoder(model_name, target_img_size=224):
    ...
    if model_name == 'resnet50_trunc':
        ...
        HAS_HOPTIMUS, HOPTIMUS_CKPT_PATH = has_HOPTIMUS()
        assert HAS_HOPTIMUS, 'HOPTIMUS is not available'
