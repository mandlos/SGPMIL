...
from src.linear_vdo import LinearVDO, Conv2dVDO
import numpy as np
...
class probabilistic_MIL_Bayes_vis(nn.Module):
    def __init__(self, gate = True, size_arg = "small", encoder:str='univ1', dropout = False, n_classes=2, top_k=1):
        ...
        if encoder == 'univ1':
            ...
            raise ValueError(f"Features from invalid encoder: {encoder}\n")
    ...
    def forward(self, h=None, validation=False):
        device = h.device
        ...
        return {"top_instance":top_instance, "Y_prob":Y_prob, "Y_hat":Y_hat, "y_probs":y_probs, "A":A}
...
class probabilistic_MIL_Bayes_enc(nn.Module):
    ...
    def forward(self, h=None, return_features=False, slide_label=None, validation=False):
        device = h.device
        #*-*# A, h = self.attention_net(h)  # NxK 
        ...
        if return_features:
            ...
            results_dict.update({'features': top_features})
        if not validation:
            return {"top_instance":top_instance, "Y_prob":Y_prob, "Y_hat":Y_hat, "kl_div":kl_div, "y_probs":y_probs, "A":A}
        else:
            return {"top_instance":top_instance, "Y_prob":Y_prob, "Y_hat":Y_hat, "y_probs":y_probs, "A":A}


class probabilistic_MIL_Bayes_spvis(nn.Module):
    def __init__(self, gate=True, size_arg="small", encoder:str='univ1', patch_size:int=224, dropout=False, n_classes=2, top_k=1):
        ...
        if encoder == 'univ1':
            ...
            self.size_dict = {"small": [1024, 512, 256], "big": [1024, 512, 384]}
        elif encoder == 'mnist_bags':
            self.size_dict = {"small": [128, 64, 32], "big": [128, 64, 32]}
        else:
            raise ValueError(f"Features from invalid encoder: {encoder}\n")
    ...
    def forward(self, h=None, coords=None, height=None, width=None, slide_label=None, validation=False):

        device = h.device
        h = F.relu(self.dp_0(self.linear1(h)))
        # print(f'shape after dp_0: {h.shape}')

        feat_a = self.dp_a(torch.sigmoid(self.linear2a(h)))
        # print(f'shape after dp_a: {feat_a.shape}')
        feat_b = self.dp_b(torch.tanh(self.linear2b(h)))
        # print(f'shape after dp_b: {feat_b.shape}')
        feat = feat_a.mul(feat_b)
        # print(f'shape after mul: {feat.shape}')
        params = self.linear3(feat)
        # print(f'Shape of params after linear3: {params[:, 0].shape}, {params[:, 1].shape}')


        coords = coords // self.patch_size
        # print(f'Coords: {coords}')
        asign = lambda coord: coord[:, 0] + coord[:, 1] * (width // self.patch_size)
        coords = asign(coords).to(device)
        # coords = torch.from_numpy(coords).to(device)
        # print(f'Coords after asign: {coords}')
        ...
        if not validation:
            mu_pr = self.prior_mu[slide_label.item()].expand_as(mu).to(device)
            logvar_pr = self.prior_logvar[slide_label.item()].to(device)
            kl_div = self.kl_logistic_normal(mu_pr, mu, logvar_pr, logvar)
        else:
            kl_div = None
        ...
        if not validation:
            return {"top_instance":top_instance, "Y_prob":Y_prob, "Y_hat":Y_hat, "kl_div":kl_div, "y_probs":y_probs, "A":patch_A.view((1, -1))}
        else:
            return {"top_instance":top_instance, "Y_prob":Y_prob, "Y_hat":Y_hat, "y_probs":y_probs, "A":patch_A.view((1, -1))}
